...
constexpr uint32_t bandwidth_hz = 500'000;  // SHORT_TURBO
constexpr uint8_t default_slot = 30;
...
class MeshtasticRxView : public View {
   public:
    MeshtasticRxView(NavigationView& nav);
    ~MeshtasticRxView();

    void focus() override;

    std::string title() const override { return "Meshtastic"; };

   private:
    NavigationView& nav_;

    /* 2 Msps: 4x oversampling of the 500 kHz chip rate, an integer decimation,
     * and a rate ADS-B RX already sustains on this hardware. */
    ...
    app_settings::SettingsManager settings_{
        "rx_meshtastic", app_settings::Mode::RX};
