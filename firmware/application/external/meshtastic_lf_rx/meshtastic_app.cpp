...
MeshtasticLfRxView::MeshtasticLfRxView(NavigationView& nav)
    : nav_{nav} {
    ...
    if (logger)
        logger->append(logs_dir / u"MESHTLF.TXT");

    text_status.set("SF7 BW500 -> /LOGS/MESHTLF.TXT");
    refresh_stats();
}

MeshtasticLfRxView::~MeshtasticLfRxView() {
    receiver_model.disable();
    baseband::shutdown();
}

void MeshtasticLfRxView::focus() {
...
void MeshtasticLfRxView::refresh_stats() {
...
void MeshtasticLfRxView::note_addr(uint32_t a) {
    for (uint8_t i = 0; i < n_addr; i++)
        if (addrs[i] == a) return;
    if (n_addr < 64) addrs[n_addr++] = a;
}

void MeshtasticLfRxView::on_packet(const ACARSPacketMessage* packet) {
    switch (packet->state) {
